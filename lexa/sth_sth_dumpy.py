...
class VideoFolder():
    ...
    def process_video(self, item):
        os.makedirs("/iris/u/nivsiyer/something_something/" + item.id)
        try: 
            reader = av.open(item.path)
        except:
            print("Issue with opening the video, path:", item.path)
            assert(False)
        vidcap = cv2.VideoCapture(item.path)
        success,image = vidcap.read()
        count = 0
        while success:
            cv2.imwrite("/iris/u/nivsiyer/something_something/%s/%s.jpg" % (str(item.id), count), image)     # save frame as JPEG file      
            success,image = vidcap.read()
            # print(image)
